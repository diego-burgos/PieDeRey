...
public class AppCrud {
    ...
    public Object[][] agregarContenidoM(LeerArchivo aq, Object[][] modelo){
        Object[][] datosAnt=listarContenido(aq);
        int tamanho = (datosAnt==null?0:datosAnt.length);
        //int numColum = numColumna(aq, modelo);
        Object[][] datosNew = new Object[tamanho+1][modelo[0].length];
        ...
        List<String> lista = new ArrayList<>();
        for (int i = 0; i < datosNew[0].length; i++) {
            datosNew[datosNew.length-1][i] = modelo[0][i];
        }
        String contenido="";
        for (int j = 0; j < datosNew.length; j++) {
            for(int k=0; k<datosNew[0].length;k++){
